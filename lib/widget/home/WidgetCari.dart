...
class _WidgetCariState extends State<WidgetCari> {
  ...
  void _getProdukByParam(key) async {
    ...
    Api.getAllProdukByParam(token, idKecamatan, idKota, idProvinsi, idSubKategori, key).then((response) {
      ...
      if (result == []) {
        ...
        setState(() {
          dataProdukList = list.map((model) => ProdukListM.fromMap(model)).toList();
        });
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      itemBuilder: (BuildContext context, int j) {
        ...
        var kecamatan = dataProdukList[j].nama_kecamatan == null ? '' : dataProdukList[j].nama_kecamatan.toLowerCase();
        var kota = dataProdukList[j].nama_kabkota == null ? '' : dataProdukList[j].nama_kabkota.toLowerCase();
        var provinsi = dataProdukList[j].nama_propinsi == null ? '' : dataProdukList[j].nama_propinsi.toLowerCase();
        return Card(
